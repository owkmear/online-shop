        ...
        private class SimpleMembershipInitializer
        {
            public SimpleMembershipInitializer()
            {
                ...
                try
                {
                    ...
                    // Проверка наличия роли заказчика (сustomer)
                    if (!roles.RoleExists("Customer"))
                    {
                        roles.CreateRole("Customer");
                    }

                    // Проверка наличия роли менеджера (manager)
                    ...
                    {
                        membership.CreateUserAndAccount("admin", "123456"); // создание пользователя
                        roles.AddUsersToRoles(new[] { "admin" }, new[] { "Manager" }); // установка роли для пользователя
                    }
                }
                catch (Exception ex)
                {
